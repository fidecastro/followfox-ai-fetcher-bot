##### Followfox AI Fetcher Bot - Version 2.2-webicon - May 31 2023
...
TOKEN = os.getenv('DISCORD_BOT_TOKEN_WEBICON')  # Fetch token from environment variable
...
MODEL_NAME = "4 - web_icons_v1_40.ckpt [2270b47c32]"

BOT_NAME = 'webicon'
    
BOT_DESCRIPTION = 'Create web icons with the power of FollowFox AI'

HI_RES_FIX=False

PRIVATE_CHANNEL_ID=1113671334229201036
...
async def create(ctx, *, prompt: str, negative_prompt: str = ""):
    # Check if the command is invoked in the private channel
    if ctx.channel.id != PRIVATE_CHANNEL_ID:
        await ctx.respond("This command can only be used in the designated private channel.")
        return
