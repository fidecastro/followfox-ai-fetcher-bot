##### Followfox AI Fetcher Bot - Version 2 - XYZ Plot implementation - May 21 2023
...
TOKEN = os.getenv('DISCORD_BOT_TOKEN_XYZ')  # Fetch token from environment variable
SERVER_ID = [1105589864453394472]  # Followfox AI Testbed
MAX_IMAGES = 1  # Maximum number of images to generate
...
async def fetch_images(input_text, input_negative_text):
    ...
    payload["script_args"] = [x_type:=1,x_values:="1,2,3,4,5",x_values_dropdown:=0,y_type:=6,y_values:="1,3,5,7,9",x_values_dropdown:=0,z_type:=10,z_values:="'1_bloody_mary_v1.safetensors','4_vodka_v3_100.ckpt'",z_values_dropdown:=["1_bloody_mary_v1.safetensors", "4_vodka_v3_100.ckpt"],draw_legend:=True, include_lone_images:=False, include_sub_grids:=False, no_fixed_seeds:=False, margin_size:=0]  # Introduce to payload the script arguments
...
@bot.slash_command(name="create-xyz", guild_ids=SERVER_ID, description="Create a XYZ plot for Bloody Mary and Vodka-v3", interaction_response_message="Creating XYZ Plot. Please wait...")
